...
def read_lexicon_magnitude(filename: str) -> Dict[str, Tuple[int, str]]:
    ...
    for line in lines[0:100]:
        ...
        magnitude_start = line.find('=', word_end, len(line)) + 1
        magnitude_end = line.find(' ', magnitude_start, len(line))
        ...
        if line[sentiment_start:sentiment_end] == "positive":
            ...
            sentiment = -1
        lexicon_data[line[word_start:word_end]] = (sentiment, line[magnitude_start:magnitude_end])
    return lexicon_data
    ...
    for word in review:
        if word in lexicon.keys():
            if lexicon[word][1] == 'strong':
                total += lexicon[word][0] * 4
            if lexicon[word][1] == 'weak':
...
def sign_test(actual_sentiments: List[int], classification_a: List[int], classification_b: List[int]) -> float:
    ...
    """
    tie, plus, minus = 0, 0, 0
    l = len(actual_sentiments)
    for i in range(l):
        if classification_a[i] == actual_sentiments[i] and classification_b[i] != actual_sentiments[i]:
            plus += 1
        elif classification_a[i] != actual_sentiments[i] and classification_b[i] == actual_sentiments[i]:
            minus += 1
        else:
            tie += 1
    tie = (tie + 1) // 2
    n = 2 * tie + plus + minus
    k = tie + min(plus, minus)
    big_x = 0
    for i in range(k + 1):
        big_x += comb(n, i) * (0.5 ** n)
    return 2 * big_x
